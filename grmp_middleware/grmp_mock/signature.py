...
class SignaturePolicy:
    ...
    def check(self, path: str, headers: Mapping[str, str], now: Optional[float] = None) -> Optional[str]:
        ...
        for name in ("appkey", "timestamp", "signature"):
...
def policy_from_args(appkey: str, public_key_text: str, user_id: Optional[str], timestamp_unit: str,
                     ...
                     read_file: Callable[[str], str] = lambda p: open(p, encoding="utf-8").read()) -> SignaturePolicy:
    ...
    return SignaturePolicy(
        appkey=appkey, public_key=sm2.parse_public_key(text),
        ...
        payload=payload, window_seconds=window,
    )
