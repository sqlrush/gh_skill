...
version: 2.0.0
description: "通过内置脚本对 OpenGauss/GaussDB 慢 SQL 做深度调优。用户要按 sql_id 或 SQL 文本优化慢 SQL、评估索引或改写方案、验证某个调优想法是否真的改善执行计划或 cost，或基于 topsql/slowsql/WDR 的结果继续调优时使用，包括“优化这条 SQL”“这个 sql_id 怎么调优”“看看建什么索引”“这个改写有没有效果”等请求。触发后运行 scripts/sqltune.py 和 scripts/verify.py，输出有证据支撑的调优结果，不要只给未经验证的优化建议。"
...
本技能用 Python 脚本（`{baseDir}/scripts/`）取数与验证：`sqltune.py` 一次性出证据包+索引验证，`verify.py` 验改写。连接元数据读取 `{baseDir}/../common/config.yaml`，凭据由脚本从 `{baseDir}/../common/credentials/` 自动解密。
