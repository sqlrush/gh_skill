...
- **只通过本技能脚本取数**：`{baseDir}/scripts/sqltune.py` 走只读会话、自动解密 `{baseDir}/../common/credentials/` 凭据，**你自己不要**直接写 Python/psql/gsql 连库、不要读取或解密 `{baseDir}/../common/credentials/`。脚本未覆盖的能力，如实说明「当前无此能力」并停止。
<!-- RED-LINES:END -->
- 脚本对数据库的会话是**只读**的（写/DDL 被会话级 `READ ONLY` 拦截）；`--analyze` 只在直连原始会话上才真正执行 SQL（DML 自动包在回滚事务里）；中间件路径的现场脚本按客户只读要求固定关闭 ANALYZE，此时报告会写明「估算计划」。
