...
连接配置目录由 `GSDB_HOME` 指定：`$GSDB_HOME/config.yaml`（无密码），口令经本机密钥 AES-256-GCM 加密存于 `$GSDB_HOME/credentials/*.enc`，由脚本解密；也可用环境变量 `GSDB_PASSWORD`（旧 `GDAA_PASSWORD` 仍兼容）覆盖。`-c <NAME>` 用 `config.yaml` 里 `connections[].name`。
