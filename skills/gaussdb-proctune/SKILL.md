...
2. **采集证据——两条命令，中途不停。**
   ...
   ```bash
   python3 {baseDir}/scripts/proctune.py collect      -c <conn> <schema.proc | schema.package.proc>   # 结构发现 + 运行时归因（建议层）
   python3 {baseDir}/scripts/proctune.py tune-cursor  -c <conn> <schema.proc | schema.package.proc>   # 每个合规游标的证据 + 索引硬验证
   ```
...
- 状态行写「模式:文件(原因)」时:环境里没配或连不上向量库/图库,这一节是**直接在知识库文件上**检索出来的——
  词法命中 `rules/ cases/ guides/ errata/`,「本行历史路径」来自 `graph/*.yaml` 里客户确认过的边。用法与上面完全一样,
  **不因为是文件模式就放松引用要求**;要补看案例全文,先读 `{kbDir}/CASES.md`(案例逐条清单)再读 `{kbDir}/cases/` 里的文件。
  「图:图文件」同理——只是路径改从文件里走,不是没有路径。
...
案例同理:`{kbDir}/CASES.md` 是案例逐条清单(ID / 系统 / 时间 / 结论强度 / 复发标志 / 文件),要案例全文时按它去 `{kbDir}/cases/` 读。
